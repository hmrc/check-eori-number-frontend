...
import uk.gov.hmrc.checkeorinumberfrontend.models.Check
import uk.gov.hmrc.checkeorinumberfrontend.views.html.templates.CheckPage
...
class CheckEoriNumberController @Inject()(
  ...
  def checkForm = Action.async { implicit request =>
  ...
  object CheckEoriNumberController {
    ...
    val form: Form[Check] = Form(
      mapping(
        "target" -> mandatoryEoriNumber("target")
